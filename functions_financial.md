...
Simple Syntax:`string GET_SYMBOLOGY_MAPPING(string dest_symbology [, string src_symbology, string symbol, datetime timestamp])````sql
...
Simple Syntax:`double CORP_ACTIONS(string field_name [, string adjustment_date, string adjust_rule, string adjustment_types])````sql
select CLOSE,
...
Simple Syntax:`string MKT_ACTIVITY(string calendar_name)````sql
...
Simple Syntax:`field type TICK_SHIFT(string field_name,int offset [,string group_by_field_names])````sql
...
Simple Syntax:`field type TIME_SHIFT(field name,shift time [,selection criteria,timestamp field])````sql
select PRICE, SIZE, EXCHANGE,
